        ...
        try {
            ...
            for (int i = 0; i < imageBytes.length; i++) {
                if (messageIndex < secretMessage.length()) {
                    ...
                    String binaryByte = String.format("%8s", Integer.toBinaryString(imageBytes[i] & 0xFF)).replace(' ', '0');
                    ...
                    imageBytes[i] = (byte) Integer.parseInt(binaryByte, 2);
            ...
            messageIndex = 0;
            for (int i = 0; i < modifiedImageBytes.length; i++) {
                if (messageIndex < secretMessage.length()) {
                    // Получаем двоичное представление текущего байта
                    String binaryByte = String.format("%8s", Integer.toBinaryString(modifiedImageBytes[i] & 0xFF)).replace(' ', '0');
